...
const Donation = () => {
  ...
  const handleDonationSubmit = async (e) => {
    ...
    try{
      ...
      const response=await axios.post("https://foodmanager-z62f.onrender.com/donate",
      donationData,
      {
        headers: {
          Authorization: token,
  ...
  useEffect(() => {
    ...
    const initializeMap = () => {
      const map = L.map('map').setView([26.862410, 81.020348], 12); // Set the initial map view
      ...
      const marker = L.marker([26.862410, 81.020348], { draggable: true }).addTo(map); // Add a draggable marker
  ...
  return (
    <div className="donation-body">
      <div className="head-description">
        <h1>Donate Food with <span className="site-name">FoodManager</span></h1>
